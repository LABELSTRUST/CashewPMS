...
<div class="row">
  ...
  <div class="col-lg-10" style="margin-left: 2%;">
      <div class="row d-flex justify-content-between">
        ...
        <div class="mb-2">
            <a href="{{ route('conditioning.index') }}" class="btn btn-lg btn-outline-info">Liste</a>
        </div>
      </div>
      <div class="row">
              ...
              <div class="">
                  ...
                  <table class="table table-bordered"  width="100%" cellspacing="0">
                      <thead>
                          <tr>
                              ...
                              <th>Sacs complets</th>
                              <th>Poids</th>
                              <th>Sacs non complets</th>
                              <th>Poids non complet</th>
                              <th>Action</th>
                              <!--th>Salary</th-->
                          </tr>
                      </thead>
                      ...
                      <tbody>
                          ...
                          @foreach ($conditionings as $conditioning)
                            <tr>
                              ...
                              <td>{{ \Carbon\Carbon::parse($conditioning->created_at)->format('d/m/Y') }}</td>
                              <td>{{$conditioning->getClassification->getUnskinning->unskinning_batch }}</td>
                              <td>{{$conditioning->num_bag}}</td>
                              <td>{{$conditioning->weight}}</td>
                              <td>{{$conditioning->remain_bag}}</td>
                              <td>{{$conditioning->remain_weight}}</td>
                              <td >
                                  <a class="btn icon-btn btn-outline-info mr-2" href="{{ route('conditioning.details',[$conditioning->id]) }} "><span class="feather icon-eye"></span></a>{{--  --}}
                              </td>
                            </tr>
                          @endforeach
                      </tbody>
                ...
                <div class="row mt-2">
                  <div class="col-lg-12 col-12">
                    <form action="{{ route('conditioning.registerRapport') }} " method="POST" enctype="multipart/form-data">
                      @csrf
