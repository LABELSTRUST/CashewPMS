                                ...
                                <table class="table table-hover table-bordered"  width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                          <td> {{ $shelling->id }}</td>
                                          <td>
                                            {{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $shelling->created_at)->format('d/m/Y') }}
                                          </td>
                                          <td>
                                            {{ $shelling->sub_batch_caliber }}
                                          </td>
                                          <td>
                                            {{ $shelling->weight }}
                                          </td>
                                          <td>
                                            @if ($shelling->transfert == 1)
                                                <button class="btn btn-success" >Transférer</button>
                                            @else
                                              <a class="btn btn-info" href=" {{ route('shelling.transfert', [$shelling->id]) }} ">Transférer </a>
                                            @endif
                                          </td>
